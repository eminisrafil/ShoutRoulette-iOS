...
- (void)failedValidation {
	NSString *alertMessage = [NSString stringWithFormat:@"Shouts should be between %i-%i characters", kSRMinPostLength, kSRMaxPostLength];
...
- (bool)validatePost:(NSString *)post {
	if (post.length > kSRMinPostLength && post.length <= kSRMaxPostLength) {
		return YES;
	}
	else {
		return NO;
...
//Prevent too many characters && returns "\n\r"
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
	...
	if (newLength <= kSRMaxPostLength) {
		return YES;
	}
	else {
		NSInteger emptySpace = kSRMaxPostLength - (textView.text.length - range.length);
